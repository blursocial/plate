...
const DEFAULT_LINE_HEIGHTS = [1, 1.2, 1.5, 2];
const DEFAULT_LINE_HEIGHT = 1.2;

const getLineHeightOverrideProps = () => editor => {
  const {
    lineHeights,
    classNames,
    defaultLineHeight,
    types
  } = plateCore.getPlatePluginOptions(editor, KEY_LINE_HEIGHT);
...
const setLineHeight = (editor, {
  ...
  const match = n => slate.Editor.isBlock(editor, n) && types.includes(n.type);

  if (lineHeight === defaultLineHeight) {
    plateCommon.unsetNodes(editor, 'lineHeight', {
      match,
      ...options
    });
  } else {
...
exports.getLineHeightOverrideProps = getLineHeightOverrideProps;
