...
const getIndentOverrideProps = () => editor => {
  const {
    types,
    classNames
...
const setIndent = (editor, {
  ...
  const nodes = Array.from(plateCommon.getNodes(editor, {
    block: true,
    ...options
  }));
  nodes.forEach(([node, path]) => {
    ...
    const blockIndent = (_node$indent = node.indent) !== null && _node$indent !== void 0 ? _node$indent : 0;
...
const onKeyDownHandler = editor => event => {
 ...
 */
const withIndent = options => editor => {
  const {
    normalizeNode
  } = editor; // TODO: extend plate-core to register options

  editor.options[KEY_INDENT] = defaults_1(options, {
    type: KEY_INDENT,
    types: [plateCore.getPlatePluginType(editor, plateCommon.ELEMENT_DEFAULT)],
    offset: 24,
    unit: 'px',
...
const createIndentPlugin = options => ({
...
exports.getIndentOverrideProps = getIndentOverrideProps;
